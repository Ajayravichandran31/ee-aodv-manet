 ...
 *
 * Based on
 *      NS-2 EEAODV model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      EEAODV-UU implementation by Erik Nordström of Uppsala University
 *      https://web.archive.org/web/20100527072022/http://core.it.uu.se/core/index.php/EEAODV-UU
 *
...
#include "eeaodv-routing-protocol.h"
...
NS_LOG_COMPONENT_DEFINE("EeaodvRoutingProtocol");

namespace eeaodv
{
NS_OBJECT_ENSURE_REGISTERED(RoutingProtocol);

/// UDP Port for EEAODV control traffic
const uint32_t RoutingProtocol::EEAODV_PORT = 654;

/**
 * \ingroup eeaodv
 * \brief Tag used by EEAODV implementation
 */
class DeferredRouteOutputTag : public Tag
{
  public:
    ...
    {
        static TypeId tid = TypeId("ns3::eeaodv::DeferredRouteOutputTag")
                                .SetParent<Tag>()
                                .SetGroupName("Eeaodv")
...
{
    static TypeId tid =
        TypeId("ns3::eeaodv::RoutingProtocol")
                             ...
                             Socket::SocketErrno& sockerr)
{
    ...
    if (m_socketAddresses.empty())
    {
        ...
        NS_LOG_LOGIC("No eeaodv interfaces");
        Ptr<Ipv4Route> route;
        return route;
    }
                            ...
                            const ErrorCallback& ecb)
{
    ...
    // EEAODV is not a multicast routing protocol
    if (dst.IsMulticast())
    {
        return false;
    }
        ...
        if (m_ipv4->GetInterfaceForAddress(iface.GetLocal()) == iif)
        {
            if (dst == iface.GetBroadcast() || dst.IsBroadcast())
            {
                ...
                if (header.GetProtocol() == UdpL4Protocol::PROT_NUMBER)
                {
                    ...
                    if (udpHeader.GetDestinationPort() == EEAODV_PORT)
                    {
                        // EEAODV packets sent in broadcast are already managed
                        return true;
                    }
                }
                if (header.GetTtl() > 1)
...
RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    {
        NS_LOG_WARN("EEAODV does not work with more then one address per each interface.");
    }
    ...
    NS_ASSERT(socket);
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvEeaodv, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetLocal(), EEAODV_PORT));
    ...
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvEeaodv, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetBroadcast(), EEAODV_PORT));
    socket->SetAllowBroadcast(true);
    socket->SetIpRecvTtl(true);
...
RoutingProtocol::NotifyAddAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("EEAODV does not work with more then one address per each interface. Ignore "
                     "added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("Remove address not participating in EEAODV operation");
    }
}
...
RoutingProtocol::LoopbackRoute(const Ipv4Header& hdr, Ptr<NetDevice> oif) const
{
    ...
    // returned when EEAODV does not have a route; this causes the packet
    // to be looped back and handled (cached) in RouteInput() method
    // while a route is found. However, connection-oriented protocols
    // like TCP need to create an endpoint four-tuple (src, src port,
    // dst, dst port) and create a pseudo-header for checksumming.  So,
    // EEAODV needs to guess correctly what the eventual source address
    // will be.
    //
    // For single interface, single address nodes, this is not a problem.
    // When there are possibly multiple outgoing interfaces, the policy
    // implemented here is to pick the first available EEAODV interface.
    ...
    NS_ASSERT_MSG(rt->GetSource() != Ipv4Address(), "Valid EEAODV source address not found");
...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    // Send RREQ as subnet directed broadcast from each interface used by eeaodv
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        TypeHeader tHeader(EEAODVTYPE_RREQ);
...
{
    socket->SendTo(packet, 0, InetSocketAddress(destination, EEAODV_PORT));
}
...
void
RoutingProtocol::RecvEeaodv(Ptr<Socket> socket)
{
    ...
    NS_LOG_DEBUG("EEAODV node " << this << " received a EEAODV packet from " << sender << " to "
                              << receiver);
    ...
    if (!tHeader.IsValid())
    {
        NS_LOG_DEBUG("EEAODV message " << packet->GetUid() << " with unknown type received: "
                                     << tHeader.Get() << ". Drop");
        return; // drop
    }
    switch (tHeader.Get())
    {
    case EEAODVTYPE_RREQ: {
        RecvRequest(packet, receiver, sender);
        break;
    }
    case EEAODVTYPE_RREP: {
        RecvReply(packet, receiver, sender);
        break;
    }
    case EEAODVTYPE_RERR: {
        RecvError(packet, sender);
        break;
    }
    case EEAODVTYPE_RREP_ACK: {
        RecvReplyAck(sender);
        break;
    }
    }
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    TypeHeader tHeader(EEAODVTYPE_RREP);
    packet->AddHeader(tHeader);
    Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(toOrigin.GetNextHop(), EEAODV_PORT));
}
                                             ...
                                             bool gratRep)
{
    ...
    if (gratRep)
    {
        ...
        TypeHeader type(EEAODVTYPE_RREP);
        packetToDst->AddHeader(type);
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toDst.GetInterface());
        NS_ASSERT(socket);
        NS_LOG_LOGIC("Send gratuitous RREP " << packet->GetUid());
        socket->SendTo(packetToDst, 0, InetSocketAddress(toDst.GetNextHop(), EEAODV_PORT));
    }
}
...
RoutingProtocol::SendReplyAck(Ipv4Address neighbor)
{
    ...
    RrepAckHeader h;
    TypeHeader typeHeader(EEAODVTYPE_RREP_ACK);
    Ptr<Packet> packet = Create<Packet>();
    SocketIpTtlTag tag;
    tag.SetTtl(1);
    packet->AddPacketTag(tag);
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(neighbor, EEAODV_PORT));
}
...
RoutingProtocol::RecvError(Ptr<Packet> p, Ipv4Address src)
{
    ...
    for (auto i = unreachable.begin(); i != unreachable.end();)
    {
        ...
        {
            TypeHeader typeHeader(EEAODVTYPE_RERR);
            Ptr<Packet> packet = Create<Packet>();
            SocketIpTtlTag tag;
            tag.SetTtl(1);
            packet->AddPacketTag(tag);
                                              ...
                                              Ipv4Address origin)
{
    ...
    packet->AddPacketTag(tag);
    packet->AddHeader(rerrHeader);
    packet->AddHeader(TypeHeader(EEAODVTYPE_RERR));
    if (m_routingTable.LookupValidRoute(origin, toOrigin))
    {
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
        NS_ASSERT(socket);
    ...
    else
    {
        for (auto i = m_socketAddresses.begin(); i != m_socketAddresses.end(); ++i)
        {
            ...
            socket->SendTo(packet->Copy(), 0, InetSocketAddress(destination, EEAODV_PORT));
        }
    }
}
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  // --- Start EE-AODV Energy Optimization ---
  double energyRatio = 1.0; // 1.0 = full battery, 0.0 = dead. Default 1.0 if no energy source (e.g. sink/server nodes).
  ...
  if (sourceContainer && sourceContainer->GetN() > 0)
    {
      ...
      if (initialEnergy > 0)
        {
          energyRatio = remainingEnergy / initialEnergy;
        }

      // 1) Hard drop only when a node is critically low (e.g. <5% battery left).
      //    This protects near-dead nodes from being used as relays at all.
      const double kCriticalEnergyRatio = 0.05;
      if (energyRatio < kCriticalEnergyRatio)
        {
          NS_LOG_LOGIC("Node " << m_ipv4->GetObject<Node>()->GetId()
                                << " energy ratio " << energyRatio
                                << " below critical threshold. Dropping RREQ.");
          return; // do not forward: protects a dying node's last reserves
        }
    }
  // energyRatio is used further below to weight the rebroadcast delay.
  // --- End EE-AODV Energy Optimization (part 1) ---
  ...
  for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
      ...
      // --- Start EE-AODV Energy Optimization (part 2: delayed rebroadcast) ---
      // Original random jitter (0-10ms), kept to avoid broadcast collisions.
      uint32_t jitterMs = m_uniformRandomVariable->GetInteger(0, 10);

      // Extra delay inversely proportional to residual energy.
      // Full battery (ratio=1.0)  -> +0ms extra delay (forwards almost immediately)
      // Near threshold (ratio~0.05) -> up to +kMaxEnergyDelayMs extra delay
      // This makes low-energy relays consistently "lose the race" to be the
      // first RREQ copy the destination/next hop sees, so AODV's normal
      // first-RREP-wins rule ends up preferring higher-energy paths.
      const double kMaxEnergyDelayMs = 25.0;
      double energyDelayMs = kMaxEnergyDelayMs * (1.0 - energyRatio);

      Time totalDelay = MilliSeconds(jitterMs) + MilliSeconds(energyDelayMs);
      ...
      Simulator::Schedule(totalDelay,
                           ...
                           packet,
                           destination);
      // --- End EE-AODV Energy Optimization (part 2) ---
    }
}
void
