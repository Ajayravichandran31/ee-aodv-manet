...
Ad Hoc On-Demand Distance Vector (EEAODV)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand
Distance Vector (EEAODV) protocol. The implementation is based on
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 EEAODV model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the EEAODV-UU implementation by Erik Nordström of
Uppsala University.

Model Description
*****************

The source code for the EEAODV model lives in the directory `src/eeaodv`.

Design
++++++

Class ``ns3::eeaodv::RoutingProtocol`` implements all functionality of
...
forwarding.  The first one, ``ns3::eeaodv::RouteOutput``, is used for
locally originated packets, and the second one, ``ns3::eeaodv::RouteInput``,
...
this functionality are attributes of ``ns3::eeaodv::RoutingProtocol``.
...
EEAODV discovers routes on demand.  Therefore, the EEAODV model buffers all
packets while a route request packet (RREQ) is disseminated.
A packet queue is implemented in eeaodv-rqueue.cc. A smart pointer to
...
* This EEAODV implementation can detect the presence of unidirectional
  ...
  This heuristic is taken from EEAODV-UU implementation and can be disabled.
...
the assertion from the EEAODV RFC that EEAODV works over UDP.  This model uses
