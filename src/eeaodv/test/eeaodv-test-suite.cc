 ...
 */
#include "ns3/eeaodv-neighbor.h"
#include "ns3/eeaodv-packet.h"
#include "ns3/eeaodv-rqueue.h"
#include "ns3/eeaodv-rtable.h"
...
{
    TypeHeaderTest()
        : TestCase("EEAODV TypeHeader")
    {
    }

    void DoRun() override
    {
        TypeHeader h(EEAODVTYPE_RREQ);
        NS_TEST_EXPECT_MSG_EQ(h.IsValid(), true, "Default header is valid");
        NS_TEST_EXPECT_MSG_EQ(h.Get(), EEAODVTYPE_RREQ, "Default header is RREQ");

        Ptr<Packet> p = Create<Packet>();
        p->AddHeader(h);
        TypeHeader h2(EEAODVTYPE_RREP);
 ...
 * \ingroup eeaodv-test
 *
 * \brief Unit test for EEAODV routing table entry
 */
...
struct EeaodvRqueueTest : public TestCase
{
    EeaodvRqueueTest()
        : TestCase("Rqueue"),
          q(64, Seconds(30))
...
EeaodvRqueueTest::DoRun()
{
    ...
    h.SetDestination(Ipv4Address("1.2.3.4"));
    h.SetSource(Ipv4Address("4.3.2.1"));
    Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback(&EeaodvRqueueTest::Unicast, this);
    Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback(&EeaodvRqueueTest::Error, this);
    ...
    Simulator::Schedule(q.GetQueueTimeout() + Seconds(1), &EeaodvRqueueTest::CheckTimeout, this);
...
void
EeaodvRqueueTest::CheckSizeLimit()
{
 ...
 * \brief Unit test for EEAODV routing table entry
 */
struct EeaodvRtableEntryTest : public TestCase
{
    EeaodvRtableEntryTest()
 ...
 * \ingroup eeaodv-test
 *
 * \brief Unit test for EEAODV routing table
 */
struct EeaodvRtableTest : public TestCase
{
    EeaodvRtableTest()
        : TestCase("Rtable")
    {
 ...
 * \ingroup eeaodv-test
 *
 * \brief EEAODV test suite
 */
class EeaodvTestSuite : public TestSuite
{
  public:
    EeaodvTestSuite()
        : TestSuite("routing-eeaodv", UNIT)
    {
        ...
        AddTestCase(new EeaodvRqueueTest, TestCase::QUICK);
        AddTestCase(new EeaodvRtableEntryTest, TestCase::QUICK);
        AddTestCase(new EeaodvRtableTest, TestCase::QUICK);
    }
} g_eeaodvTestSuite; ///< the test suite
