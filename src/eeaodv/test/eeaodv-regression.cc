...
#include "eeaodv-regression.h"

#include "bug-772.h"

#include "ns3/abort.h"
#include "ns3/eeaodv-helper.h"
 ...
 * \ingroup eeaodv-test
 *
 * \brief EEAODV regression test suite
 */
class EeaodvRegressionTestSuite : public TestSuite
{
  public:
    EeaodvRegressionTestSuite()
        : TestSuite("routing-eeaodv-regression", SYSTEM)
    {
        SetDataDir(NS_TEST_SOURCEDIR);
        // General RREQ-RREP-RRER test case
        AddTestCase(new ChainRegressionTest("eeaodv-chain-regression-test"), TestCase::QUICK);
...
} g_eeaodvRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest(const char* const prefix,
    ...
    : TestCase("EEAODV chain regression test"),
      m_nodes(nullptr),
      m_prefix(prefix),
      m_time(t),
      m_size(size),
...
ChainRegressionTest::CreateDevices()
{
    ...
    // 2. Setup TCP/IP & EEAODV
    EeaodvHelper eeaodv; // Use default parameters here
    InternetStackHelper internetStack;
    internetStack.SetRoutingHelper(eeaodv);
    ...
    streamsUsed += eeaodv.AssignStreams(*m_nodes, streamsUsed);
    // EEAODV uses m_size more streams
